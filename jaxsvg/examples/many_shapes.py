...
target = (imageio.imread(os.path.join(root_dir, "target.png")) / 255)[..., :3]


def train_step(optimizer, opt_state, p, z):

  def loss_fn(params):
    bezier_parameters, color_parameters = model.apply({'params': params}, z)
    print(bezier_parameters.shape)
    print(color_parameters.shape)
    print(color_parameters)
    raster = draw.compose_paths(bezier_parameters[0],
                                color_parameters[0],
                                sampling_res=32)
...
class ZToBezierParam(nn.Module):
  ...
  def __call__(self, z):
    ...
    color_parameters = nn.DenseGeneral(features=4,
                                       name='shape_color')(per_shape_z)
    color_parameters = nn.sigmoid(color_parameters)
    ...
    return bezier_parameters, color_parameters
...
if __name__ == "__main__":
  ...
  for i in range(128):
    ...
    if i > 1:
      writer.append_data(
          (onp.array(metrics['raster'] * 255).astype(onp.uint8)))
    print(metrics['loss'])
